...
def translate_en_to_zh(text: str, name_map: Dict[str, str]) -> str:
    ...
    system_msg = (
        ...
        "  # PI confirmation questions — keep the hypothesis-first structure.\n"
        "  'I'd expect X to go in the Y — does that match how you organise things?' -> '我猜X应该放在Y——这和你整理东西的方式一致吗？'\n"
        "  'I'd expect X to go in the Y — is that how you like to organise things?' -> '我猜X应该放在Y——这是你喜欢的整理方式吗？'\n"
        "  'I think X belong(s) in the Y — is that right?' -> '我觉得X应该放在Y——对吗？'\n"
        "  'I'd guess X go(es) in the Y — does that match?' -> '我猜X应该放在Y——你认可吗？'\n"
        "  规则：当原文以 \"I'd expect / I'd guess / I think\" 开头时，必须保留\"我猜/我觉得\"+\"X 应该放在 Y\"的假设式句式，不要改成\"Y 里你一般放什么\"或\"X 你一般放在哪\"等开放式问法。\n"
        "  room 词翻译表：kitchen/bedroom/living room→厨房/卧室/客厅；"
        "study_desk→书桌；bar_kitchen→厨房；fridge→冰箱；bedroom_practice→卧室。\n"
        "- 以下术语必须使用对应中文：\n"
        f"{glossary}"
    )
    try:
