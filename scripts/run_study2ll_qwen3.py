"""Run Study 2 LL (LLM-driven selection, Study2 PI/PE proposers) on Qwen3.

Same metric format as scripts/run_ll_gpt5chat.py — full QA + state per
(episode, budget) checkpoint — but uses Study2QuestionPolicyController so
turn-1 AO/PI choice is LLM-driven, not the Study 1 rule path.
"""
...
# Set env BEFORE importing llm_factory.
os.environ.setdefault("LLM_BACKEND", "ollama")
os.environ.setdefault("LLM_MODEL", "qwen3")
os.environ.setdefault("LLM_BASE_URL", "http://110.42.252.68:8080")
os.environ.setdefault("OLLAMA_HOST", "http://110.42.252.68:8080")
...
BUDGETS = [1, 3, 10]
MODE = "learner_led"
LOG_PATH = PROJECT_ROOT / "logs" / "study2ll_qwen3_10ep.jsonl"
...
def run_one(episode, *, controller, ao, pe, pi, updater, oracle, planner):
    """Run up to max(BUDGETS) turns, evaluate at each budget checkpoint.
    Returns dict {budget: evaluation}."""
    state: AgentState = build_initial_state(
        episode=episode, strategy="learner_led", budget_total=max(BUDGETS),
    )
    ...
    while len(state["qa_history"]) < max(BUDGETS):
        ...
        if b in eval_set:
            ...
            results[b] = ev
    # fill any unreached budgets (early stop)
    for b in BUDGETS:
        if b not in results:
            ev = evaluate_episode_state(episode, state, planner=planner)
            ev["qa_history"] = list(state["qa_history"])
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        try:
            ...
            print(f"[ep_idx={index}] DONE ({elapsed:.0f}s, attempt={attempt}, fb={fb}) "
                  f"u@1={results[1]['unseen_accuracy']*100:.0f} "
                  f"u@3={results[3]['unseen_accuracy']*100:.0f} "
                  f"u@10={results[10]['unseen_accuracy']*100:.0f}",
                  flush=True)
            return True
...
def main():
    ...
    if not LOG_PATH.exists():
        _append({
            ...
            "budgets": BUDGETS,
            "mode": MODE,
            "controller": "Study2QuestionPolicyController (LLM-driven LL)",
            "model": DEFAULT_MODEL,
        })
    components = {
