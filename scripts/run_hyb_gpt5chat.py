"""Run HYB on GPT-5-Chat with full QA logging.

HYB = Hybrid: all three patterns (PE/AO/PI) available, parent _rule_hybrid
adaptively switches. Uses Study2 PE proposer (debiased) + parent PI proposer.
...
BUDGETS = [1, 3, 5, 10]
MODE = "hybrid"
LOG_PATH = PROJECT_ROOT / "logs" / "hyb_gpt5chat_5ep_b1-3-5-10.jsonl"
...
def main():
    ...
    if not LOG_PATH.exists():
        _append({
            ...
            "budgets": BUDGETS,
            "mode": MODE,
            "controller": "QuestionPolicyController (parent, HYB via _rule_hybrid)",
            "pe_proposer": "Study2PreferenceElicitingProposer",
            "pi_proposer": "PreferenceInductionProposer (parent)",
            "model": DEFAULT_MODEL,
        })
    components = {
